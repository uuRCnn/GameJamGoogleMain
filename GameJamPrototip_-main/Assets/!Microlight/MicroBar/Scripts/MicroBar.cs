...
// ****************************************************************************************************
namespace _Microlight.MicroBar.Scripts
{
  internal enum HealthBarType
  {
    SpriteRenderer,
    Image
  }
  ...
  public class MicroBar : MonoBehaviour
  {
    // Variables
    //Type
    [SerializeField] private HealthBarType _barType;
    ...
    //Colors
    [SerializeField]                 private Color _barPrimaryColor   = new(1f, 1f, 1f); // Color of main health bar
    [SerializeField]                 private Color _barGhostHurtColor = new(1f, 0f, 0f); // Color of ghost bar when hurt
    [SerializeField]                 private Color _barGhostHealColor = new(1f, 1f, 1f); // Color of ghost bar when healed
    [SerializeField]                 private Color _barAdaptiveColor  = new(1f, 0f, 0f); // Color that health changes to as it gets lower
    [SerializeField]                 private Color _barFlashingColor  = new(1f, 0f, 0f); // Color that health changes to as it flashes
    [SerializeField] [Range(0f, 1f)] private float _ghostBarAlpha     = 0.6f;

    //General Settings
    [SerializeField] private bool _adaptiveColor = true; // Does health bar uses adaptive color based on current hp or single color for each bar
    [SerializeField] private bool _flashingLowHealth; // Will health bar flash in different color when low
    [SerializeField] private bool _isAnimated  = true; // Is health bar animated
    [SerializeField] private bool _useGhostBar = true; // Is ghost bar used    
    [SerializeField] private bool _dualGhostBars; // Are ghost bars two separate bars for healing and damaging or single bar for both

    //Flashing Animation
    [SerializeField] [Range(0f, 100f)] private float _lowHealthThreshold = 25f; // How low health needs to be from max HP (in %) to be considered low HP
    [SerializeField] [Range(0f, 1f)]   private float _flashToTime        = 0.1f; // Time required to change color from default to low health color
    [SerializeField] [Range(0f, 1f)]   private float _flashFromTime      = 0.3f; // Time requred to change color back to default from low health color

    //Bar Animation
    [SerializeField] [Range(0.01f, 2f)] private float _barFillDuration = 0.5f; // Duration of bar moving
    [SerializeField] [Range(0f, 2f)]    private float _damageFillDelay; // How long will fill animation wait after shake animation
    [SerializeField] [Range(0f, 2f)]    private float _healFillDelay; // How long will fill animation wait before animating healing
    [SerializeField] [Range(0f, 20f)]   private float _animationTriggerThreshold = 1f; // % of HP that needs to change for animations to trigger  

    // Damage Shake
    [SerializeField]                    private bool    _damageShake = true; // Will bar shake during damage
    [SerializeField]                    private bool    _adaptiveDamageShake; // Will shake be adaptive based on change        
    [SerializeField] [Range(0.01f, 5f)] private float   _damageShakeAmount      = .2f; // Bar shake amount, how much it moves around screen
    [SerializeField] [Range(1, 200)]    private int     _damageShakeIntensity   = 40; // Intensity of damage shake
    [SerializeField] [Range(0.01f, 2f)] private float   _damageShakeDuration    = 0.3f; // How long damage shake will last
    [SerializeField]                    private Vector2 _adaptiveShakeAmount    = new(0f, 0.3f); // Bar shake amount based on how much of health bar has been taken out
    [SerializeField]                    private Vector2 _adaptiveShakeIntensity = new(10, 50); // Bar shake intensity based on how much of health bar has been taken out
    [SerializeField] [Range(0.1f, 1f)]  private float   _adaptiveShakeThreshold = .5f; // % of bar that needs to change to reach full adaptive value

    // Heal Scale
    [SerializeField]                    private bool    _healScale = true; // Will bar scale during healing
    [SerializeField]                    private bool    _adaptiveHealScale; // Will scale be adaptive based on change        
    [SerializeField] [Range(1.01f, 2f)] private float   _healScaleAmount            = 1.3f; // How much health bar should increase during healing
    [SerializeField]                    private Vector2 _adaptiveHealScaleAmount    = new(1f, 1.3f); // Heal scale amount based on how much of health bar has been healed
    [SerializeField] [Range(0.1f, 1f)]  private float   _adaptiveHealScaleThreshold = .5f; // % of bar that needs to change to reach full adaptive value
    private                                     Color   _backgroundBarColor; // Stores color of background bar
    private                                     float   _change; // Difference between old and new bar position (change is in %)
    private                                     float   _fillAmount = 1f; // How much bar is filled (%)
    private                                     bool    _isDamage   = true; // Is current animation damage or heal
    private                                     bool    _isInitalized; // Safety system that gives out warning if health bar has not been initialized
    private                                     float   _maxValue                = 1f; // Max value of HP
    private                                     float   _overrideHealScaleAmount = -1f;

    private       float          _overrideShakeAmount    = -1f;
    private       float          _overrideShakeDuration  = -1f;
    private       int            _overrideShakeIntensity = -1;
    private       float          _value                  = 1f; // Current value of HP
    public static List<MicroBar> AnimatedHealthBars { get; private set; } = new();
    ...
    // Tweens
    private Tween                        BarTween              { get; set; }
    private Sequence                     BarFlashingColorTween { get; set; }
    private Tween                        ShakeTween            { get; set; }
    private Tween                        HealTween             { get; set; }
    private Tween                        BackgroundFadeTween   { get; set; }
    private Tween                        BarFadeTween          { get; set; }
    private Tween                        GhostBarFadeTween     { get; set; }
...
#region Public Methods
      ...
      public void SetMaxHealth(float maxValue)
    {
      ...
      /// <param name="disableAnimate">
      ///   If true, disables animation regardless of options and just
      ///   updates bar
      /// </param>
      public void UpdateHealthBar(float value, bool disableAnimate = false)
    {
      ...
      else
      {
        ...
        var currentValue = _barType == HealthBarType.SpriteRenderer ? _srPrimaryBar.size.x : _uiPrimaryBar.fillAmount;
      ...
      public void FadeBar(bool fadeIn, float duration)
    {
      ...
      var backgroundFadeTo = fadeIn ? _backgroundBarColor.a : 0f;
      var barFadeTo = fadeIn ? 1f : 0f;
      var ghostFadeTo = fadeIn ? _ghostBarAlpha : 0f;
      ...
      public MicroBar OverrideShake(float adaptiveAmount,
                                    int   adaptiveIntensity,
...
#region Local
    ...
    private void SetBarsColor()
    {
      ...
      {
        var lowHealth = _value / MaxValue * 100 <= _lowHealthThreshold; // Is health considered low
...
#region Animations
    ...
    {
      var delay = _isDamage ? _damageFillDelay : _healFillDelay; // Decide delay
    ...
    private void DamageShake()
    {
      ...
      // Override settings
      var shakeAmount = _overrideShakeAmount < 0f ? _damageShakeAmount : _overrideShakeAmount;
      var shakeIntensity = _overrideShakeIntensity < 0f ? _damageShakeIntensity : _overrideShakeIntensity;
      var shakeDuration = _overrideShakeDuration < 0f ? _damageShakeDuration : _overrideShakeDuration;
      ...
      if (_barType == HealthBarType.SpriteRenderer)
      {
        var barTransform = _srPrimaryBar.GetComponent<Transform>();
        AnimatedHealthBars.Add(this);
      ...
      {
        var barRectTransform = _uiPrimaryBar.GetComponent<RectTransform>();
        AnimatedHealthBars.Add(this);
...
#region Utility
    ...
    private void HealScale()
    {
      if (!_healScale) return; // Don't scale if option is disabled

      // Override settings
      var scaleAmount = _overrideHealScaleAmount < 0f ? _healScaleAmount : _overrideHealScaleAmount;
...
#region Utility
    ...
    private void SetAlpha(SpriteRenderer renderer, float value)
    {
    ...
    {
      // Get prefab
      var go = MicroBarAssetUtilities.GetPrefab("SpriteRendererMicroBar");
      if (go == null) return;
    ...
    {
      // Get prefab
      var go = MicroBarAssetUtilities.GetPrefab("UIImageMicroBar");
      if (go == null) return;
  ...
  public class MicroBar_Editor : Editor
  {
    ...
    */
    ...
    public override void OnInspectorGUI()
    {
      ...
      var barType = serializedObject.FindProperty("_barType");
      var adaptiveColor = serializedObject.FindProperty("_adaptiveColor");
      var flashingLowHealth = serializedObject.FindProperty("_flashingLowHealth");
      var isAnimated = serializedObject.FindProperty("_isAnimated");
      var ghostBar = serializedObject.FindProperty("_useGhostBar");
      var dualGhostBars = serializedObject.FindProperty("_dualGhostBars");
      var damageShake = serializedObject.FindProperty("_damageShake");
      var healScale = serializedObject.FindProperty("_healScale");
      var adaptiveDamageShake = serializedObject.FindProperty("_adaptiveDamageShake");
      var adaptiveHealScale = serializedObject.FindProperty("_adaptiveHealScale");
      ...
      if (!flashingLowHealth.boolValue)
      {
      ...
      if (isAnimated.boolValue)
        EditorGUILayout.PropertyField(ghostBar, new GUIContent("Use Ghost Bar",
          "Will second health bar be displayed during animation? (showing amount healed and amount hurt)."));
      if (isAnimated.boolValue && ghostBar.boolValue && !adaptiveColor.boolValue)
        EditorGUILayout.PropertyField(dualGhostBars, new GUIContent("Dual Ghost Bars",
      ...
      if (isAnimated.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_barFillDuration"), new GUIContent("Fill Duration",
          "Duration for bar to reach set value."));
      if (isAnimated.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_damageFillDelay"), new GUIContent("Damage Fill Delay",
          "Delay before fill animation starts."));
      if (isAnimated.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_healFillDelay"), new GUIContent("Heal Fill Delay",
          "Delay before fill animation starts."));
      if (isAnimated.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_animationTriggerThreshold"), new GUIContent("Animation Trigger Threshold",
      ...
      if (flashingLowHealth.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_lowHealthThreshold"), new GUIContent("Low Health Threshold",
          "At which point is health considered as low health (%)."));
      if (flashingLowHealth.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_flashToTime"), new GUIContent("Flash To Time",
          "Time to change from default color to flashing color."));
      if (flashingLowHealth.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_flashFromTime"), new GUIContent("Flash From Time",
      ...
      if (isAnimated.boolValue)
        EditorGUILayout.PropertyField(damageShake, new GUIContent("Shake",
          "Will health bar shake during damage."));
      if (damageShake.boolValue)
        EditorGUILayout.PropertyField(adaptiveDamageShake, new GUIContent("Adaptive Damage Shake",
          "Allows to change shake settings based on % of health bar changed."));
      if (damageShake.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_damageShakeDuration"), new GUIContent("Damage Shake Duration",
          "How long will damage shake last (in seconds)."));
      // Non-adaptive
      if (damageShake.boolValue && !adaptiveDamageShake.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_damageShakeAmount"), new GUIContent("Damage Shake Amount",
          "How much will bar move around during damage shake."));
      if (damageShake.boolValue && !adaptiveDamageShake.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_damageShakeIntensity"), new GUIContent("Damage Shake Intensity",
          "Vibration of damage shake."));
      // Adaptive
      if (adaptiveDamageShake.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_adaptiveShakeAmount"), new GUIContent("Adaptive Damage Shake Amount",
          "Lowest shake amount when bar changes 0% and largest shake amount when bar changes threshold % amount."));
      if (adaptiveDamageShake.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_adaptiveShakeIntensity"), new GUIContent("Adaptive Damage Shake Intensity",
          "Lowest shake intensity when bar changes 0% and largest shake intensity when bar changes threshold % amount."));
      if (adaptiveDamageShake.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_adaptiveShakeThreshold"), new GUIContent("Adaptive Damage Shake Threshold",
      ...
      if (isAnimated.boolValue)
        EditorGUILayout.PropertyField(healScale, new GUIContent("Heal Scale",
          "Will health bar scale up during healing."));
      if (healScale.boolValue)
        EditorGUILayout.PropertyField(adaptiveHealScale, new GUIContent("Adaptive Heal Scale",
          "Allows to change shake settings based on % of health bar changed."));
      // Non-adaptive
      if (healScale.boolValue && !adaptiveHealScale.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_healScaleAmount"), new GUIContent("Heal Scale Intensity",
          "How much health bar will scale up."));
      // Adaptive
      if (adaptiveHealScale.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_adaptiveHealScaleAmount"), new GUIContent("Adaptive Heal Scale Amount",
          "Lowest scale amount when bar changes 0% and largest scale amount when bar changes threshold % amount."));
      if (adaptiveHealScale.boolValue)
        EditorGUILayout.PropertyField(serializedObject.FindProperty("_adaptiveHealScaleThreshold"), new GUIContent("Adaptive Heal Scale Threshold",
